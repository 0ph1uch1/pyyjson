...
class TestFragment:
    def test_fragment_fragment_eq(self):
        assert pyyjson.Fragment(b"{}") != pyyjson.Fragment(b"{}")

    def test_fragment_fragment_not_mut(self):
        fragment = pyyjson.Fragment(b"{}")
        with pytest.raises(AttributeError):
            fragment.contents = b"[]"
        assert pyyjson.dumps(fragment) == b"{}"

    def test_fragment_repr(self):
        assert repr(pyyjson.Fragment(b"{}")).startswith("<pyyjson.Fragment object at ")

    def test_fragment_fragment_bytes(self):
        assert pyyjson.dumps(pyyjson.Fragment(b"{}")) == b"{}"
        assert pyyjson.dumps(pyyjson.Fragment(b"[]")) == b"[]"
        assert pyyjson.dumps([pyyjson.Fragment(b"{}")]) == b"[{}]"
        assert pyyjson.dumps([pyyjson.Fragment(b'{}"a\\')]) == b'[{}"a\\]'

    def test_fragment_fragment_str(self):
        assert pyyjson.dumps(pyyjson.Fragment("{}")) == b"{}"
        assert pyyjson.dumps(pyyjson.Fragment("[]")) == b"[]"
        assert pyyjson.dumps([pyyjson.Fragment("{}")]) == b"[{}]"
        assert pyyjson.dumps([pyyjson.Fragment('{}"a\\')]) == b'[{}"a\\]'

    def test_fragment_fragment_str_empty(self):
        assert pyyjson.dumps(pyyjson.Fragment("")) == b""

    def test_fragment_fragment_str_str(self):
        assert pyyjson.dumps(pyyjson.Fragment('"str"')) == b'"str"'

    def test_fragment_fragment_str_emoji(self):
        assert pyyjson.dumps(pyyjson.Fragment('"🐈"')) == b'"\xf0\x9f\x90\x88"'

    def test_fragment_fragment_str_array(self):
        n = 8096
        obj = [pyyjson.Fragment('"🐈"')] * n
    ...
    def test_fragment_fragment_str_invalid(self):
        with pytest.raises(pyyjson.JSONEncodeError):
            pyyjson.dumps(pyyjson.Fragment("\ud800"))  # type: ignore
    ...
    def test_fragment_fragment_none(self):
        with pytest.raises(pyyjson.JSONEncodeError):
            pyyjson.dumps([pyyjson.Fragment(None)])  # type: ignore

    def test_fragment_fragment_args_zero(self):
        with pytest.raises(TypeError):
            pyyjson.dumps(pyyjson.Fragment())

    def test_fragment_fragment_args_two(self):
        with pytest.raises(TypeError):
            pyyjson.dumps(pyyjson.Fragment(b"{}", None))  # type: ignore

    def test_fragment_fragment_keywords(self):
        with pytest.raises(TypeError):
            pyyjson.dumps(pyyjson.Fragment(contents=b"{}"))  # type: ignore

    def test_fragment_fragment_arg_and_keywords(self):
        with pytest.raises(TypeError):
            pyyjson.dumps(pyyjson.Fragment(b"{}", contents=b"{}"))  # type: ignore
    ...
    def test_fragment_pandas(self):
        ...
        def default(value):
            if isinstance(value, pandas.DataFrame):
                return pyyjson.Fragment(value.to_json(orient="records"))
            raise TypeError

        val = pandas.DataFrame({"foo": [1, 2, 3], "bar": [4, 5, 6]})
        assert (
            pyyjson.dumps({"data": val}, default=default)
...
class TestFragmentParsing:
    def _run_test(self, filename: str):
        ...
        pyyjson.dumps(pyyjson.Fragment(data))
