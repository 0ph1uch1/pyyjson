...
class TestDataclass:
    def test_dataclass(self):
        ...
        assert pyyjson.dumps(obj) == b'{"name":"a","number":1,"sub":null}'
    ...
    def test_dataclass_empty(self):
        ...
        dumps() no attributes
        """
        assert pyyjson.dumps(EmptyDataclass()) == b"{}"

    def test_dataclass_empty_slots(self):
        """
        dumps() no attributes slots
        """
        assert pyyjson.dumps(EmptyDataclassSlots()) == b"{}"
    ...
    def test_dataclass_default_arg(self):
        ...
        """
        obj = Dataclass2()
        assert pyyjson.dumps(obj) == b'{"name":"?"}'
    ...
    def test_dataclass_metadata(self):
        ...
        assert pyyjson.dumps(obj) == b'{"a":"a","b":1,"c":2.1}'
    ...
    def test_dataclass_classvar(self):
        ...
        """
        obj = Dataclass4("a", 1)
        assert pyyjson.dumps(obj) == b'{"a":"a","b":1,"c":1.1}'
    ...
    def test_dataclass_default(self):
        ...
        assert (
            pyyjson.dumps(obj, default=default)
    ...
    def test_dataclass_under(self):
        ...
        assert pyyjson.dumps(obj) == b'{"ab":"zxc vbn"}'
    ...
    def test_dataclass_option(self):
        ...
        obj = Dataclass1("a", 1, None)
        assert (
            pyyjson.dumps(obj, option=pyyjson.OPT_SERIALIZE_DATACLASS)
...
class TestDataclassPassthrough:
    def test_dataclass_passthrough_raise(self):
        ...
        obj = Dataclass1("a", 1, None)
        with pytest.raises(pyyjson.JSONEncodeError):
            pyyjson.dumps(obj, option=pyyjson.OPT_PASSTHROUGH_DATACLASS)
        with pytest.raises(pyyjson.JSONEncodeError):
            pyyjson.dumps(
                InitDataclass("zxc", "vbn"), option=pyyjson.OPT_PASSTHROUGH_DATACLASS
            )
    ...
    def test_dataclass_passthrough_default(self):
        ...
        obj = Dataclass1("a", 1, None)
        assert (
            pyyjson.dumps(obj, option=pyyjson.OPT_PASSTHROUGH_DATACLASS, default=asdict)
            == b'{"name":"a","number":1,"sub":null}'
        )
        ...
        assert (
            pyyjson.dumps(obj, option=pyyjson.OPT_PASSTHROUGH_DATACLASS, default=default)
            == b'{"name":"a","number":1}'
        )
...
class TestAbstractDataclass:
    def test_dataclass_abc(self):
        obj = ConcreteAbc(1.0)
        assert pyyjson.dumps(obj) == b'{"attr":1.0}'
