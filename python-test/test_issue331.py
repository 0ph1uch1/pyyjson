...
def test_issue331_1_pretty():
    ...
    as_obj = pyyjson.loads(as_bytes)
    for _ in range(1000):
        assert pyyjson.loads(pyyjson.dumps(as_obj, option=pyyjson.OPT_INDENT_2)) == as_obj
...
def test_issue331_1_compact():
    ...
    as_obj = pyyjson.loads(as_bytes)
    for _ in range(1000):
        assert pyyjson.loads(pyyjson.dumps(as_obj)) == as_obj
...
def test_issue335_compact():
    for _ in range(1000):
        assert pyyjson.dumps(FIXTURE_ISSUE_335)


def test_issue335_pretty():
    for _ in range(1000):
        assert pyyjson.dumps(FIXTURE_ISSUE_335, option=pyyjson.OPT_INDENT_2)
